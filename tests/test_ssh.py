...
def test_run_command(
    connection: base.Connection, log_stream: StringIO, test_str: str
) -> None:
...
def test_put(
    connection: base.Connection, tmp_file: IO[Any], test_str: str, test_filename: str
) -> None:
    with tmp_file:
        connection.put(src=tmp_file.name, dest=test_filename)
        file_content = connection.run_command(
            f"cat {test_filename}", capture_output=True
        )
        assert test_str == file_content


def test_get(connection: base.Connection, test_str: str, test_filename: str) -> None:
